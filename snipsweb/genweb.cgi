...
if (@row_data > $max_table_rows) {
  ...
  if ($entries == 1) {
    my $msg = "The Network is <U>healthy</U>!";
...
EOT1
    ...
    # this invokes the genweb-cfg.cgi script for customizing this view
    print "<TD valign=middle><FORM action=\"genweb2-config.cgi?$newq\" method=\"get\">
...
sub get_row_data {
  ...
  my $file=$dfile;
  $file =~ s/-output//;
  return if (defined($filepat) && $file !~ /$filepat/);
