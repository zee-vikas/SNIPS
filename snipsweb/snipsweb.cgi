...
use vars qw (
	     ...
	     $command $devicename $deviceaddr $variable $done_footer
...
sub print_button_header {
  ...
  print <<EoHeader;
  <html>
    <head><title>Snips View $FORM{'devicename'} </title></head>
      <body bgcolor="#ffffff">
	<center>
	  <FORM action="$snipsweb_cgi" method="post">
...
sub updateStatus {
  ...
  foreach (@list) {
    next if (/^$devicename\:$deviceaddr\:$variable/);	# delete/skip old entry
    print SFILE;
  }
  if ($FORM{'subcommand'} =~ /^clear/i) {
    ...
    print SFILE "$devicename\:$deviceaddr\:$variable\t";
...
sub doGraph {
  ...
  $rrddir = "$RRD_DBDIR/" . substr($deviceaddr, 0, 1) . "/$deviceaddr";

  print "<center><H3>$devicename : $deviceaddr</H3>\n",
        "  <hr width=\"50\%\" align=center>\n</center>\n";
  print <<Eof1;
   <p>Click
    <A href="${rrdgraph_cgi}?rrdsubdir=$deviceaddr&title=${devicename}"> HERE </A>
      to display <b>all variables </b>for $devicename\n</p>
   <p> Click on the image to display <b>historical</b> data for the variable</p>
Eof1

  if ( (! -d "$rrddir") || (! -r "$rrddir") || (! opendir(DATADIR, $rrddir)) )
  {
    print "<p><font size=4> <b>No RRD data available under $rrddir</b></font></p>\n";
    print STDERR "Cannot opendir($rrddir) - $!\n";
    return;
  }

  my @rrdfiles = sort grep(/\.rrd$/i, readdir(DATADIR));
  closedir (DATADIR);
  for $rrdfile (@rrdfiles)
  {
    chomp $rrdfile;
    #$devicename =~ tr/[a-zA-Z0-9_.\-]//cd;	# strip unwanted characters
    my $var = $rrdfile;
    #$rrdfile =~ s/\+/\%2b/g;	# replace all '+' in filename
    $var =~ s/\.rrd$//;
    next if (($variable ne "") && ($var ne $variable)); # only for $variable
    $var =~ tr/[a-zA-Z0-9_.\-]//cd;
    print <<Eof;
    <h4>${var}</h4>
    <A href="${rrdgraph_cgi}?rrdsubdir=$deviceaddr&rrdfile=${rrdfile}&title=${devicename}&legend=${var}&timescale=A&mode=html">
    <IMG SRC="${rrdgraph_cgi}?rrdsubdir=$deviceaddr&rrdfile=${rrdfile}&title=${devicename}%20(${var})&legend=${var}&timescale=d&mode=gif" >
    </A>
    <hr>
Eof
  }	# for $rrdfile
